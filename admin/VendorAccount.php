...
$query = mysqli_query($conn, "SELECT * FROM accounts WHERE UserType=1");

  $count = mysqli_num_rows($query);
?>

<div class="table-container">
  <div class="t-header">حسابات البائعين</div>
      ...
      while($row=mysqli_fetch_assoc($result))
         ...
         if($type=="1")
          ...
          $usert="بائع";
           ...
           if($row['UserPhoto']!="")
           {
             $photo='<td> <div class="avatar xl"> <img src="../images/user/' .$row['UserPhoto'].'"  class="circle"></div></td>';
           }
           else
           {
...
<table id="copy-print-csv" class="table custom-table">
      ...
      $sql = 'SELECT * FROM accounts WHERE UserType=1 ';
     $result=mysqli_query($conn,$sql);
      while($row=mysqli_fetch_assoc($result))
          {
     ...
     <a href="VendorAccountDetails.php?id=<?php echo $row['UserID']; ?>" class="btn btn-info">
                            تفاصيل</a>
      </td>
     </tr>
    <?php
